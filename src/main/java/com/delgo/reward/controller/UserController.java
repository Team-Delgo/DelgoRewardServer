...
public class UserController extends CommController {
    ...
    public ResponseEntity<?> registerUserByOAuth(@Validated @RequestPart(value = "data") OAuthSignUpDTO oAuthSignUpDTO, @RequestPart MultipartFile profile, HttpServletResponse response) {
        ...
        User modifyUser = userService.save(user.setProfile(photoService.uploadProfile(user.getUserId(), profile))); // User Profile 등록
        ...
        UserResDTO returnDto = new UserResDTO(modifyUser, pet);
