...
public class PhotoService extends CommService {
    ...
    public List<String> createCertPhotoList(int certificationId, List<MultipartFile> photoList) {
        AtomicInteger autoGeneratedId = new AtomicInteger(1);
        return photoList.stream().map(photo -> {
                    String fileName = certificationId + "_cert_" + autoGeneratedId.incrementAndGet() + "." + getExtension(photo);
                    return save(fileName, photo);
                }).toList();
    }
