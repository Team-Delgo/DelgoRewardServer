...
public interface UserJpaRepository extends JpaRepository<UserEntity, Integer> {
    ...
    Optional<UserEntity> findByAppleUniqueNo(String appleUniqueNo);
...
//    if Pet Name contain searchWord, 두 길이 차이의 절대값을 구한다.
    ...
    @Query("SELECT u FROM UserEntity u WHERE u.name LIKE CONCAT('%', :keyword, '%') or u.pet.name LIKE CONCAT('%', :keyword, '%') " +
            "ORDER BY CASE " +
            ...
            "ELSE 100 END")
    Slice<UserEntity> searchByName(@Param("keyword") String keyword, Pageable pageable);
}