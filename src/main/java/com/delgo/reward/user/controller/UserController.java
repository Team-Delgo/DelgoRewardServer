...
public class UserController extends CommController {
    ...
    public ResponseEntity<?> getOtherUser(@RequestParam int userId) {
        return SuccessReturn(AccountResponse.fromToOther(userService.getUserById(userId), userService.getActivityByUserId(userId), certService.getVisitedMungpleIdListTop3ByUserId(userId)));
    }
    ...
    @ApiResponse(responseCode = "200", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = PageCustomSearchUserResponse.class))})
    @GetMapping("/search")
